...
createServer(async (req, res) => {
    ...
    // Get data - FIXED FILTERING
    if (pathname === '/api/sensors/data' && req.method === 'GET') {
        let filteredData = sensorData;

        // Apply deviceId filter if provided
        if (query.deviceId) {
            filteredData = sensorData.filter(d => d.deviceId === query.deviceId);
            console.log(`Filtering for deviceId: ${query.deviceId}, found ${filteredData.length} records`);
        }
        ...
        res.end(JSON.stringify({
            ...
            data: filteredData.slice(-50) // Return last 50 matching records
        }));
        return;
    }
    ...
    if (pathname === '/' && req.method === 'GET') {
        ...
        res.end(JSON.stringify({
            ...
            endpoints: {
                'GET /api/sensors/data': 'Get all sensor data',
                'GET /api/sensors/data?deviceId=xxx': 'Get data for specific device',
